...
vidcap = cv2.VideoCapture("ledhz.mp4")                                                  # open video file called "....mp4"
...
lower = np.array([207,210,200])                                                       # mask for checking if frame has LED is ON or not
upper = np.array([255,255,255])
bgr = [240, 252, 252]                                                                 # BGR values of led in ON state
thresh = [6, 3, 3]
while success:
    success,frame = vidcap.read()                                                     # read a frame
    #cv2.imshow('frame',frame)
    tt = vidcap.get(cv2.CAP_PROP_POS_MSEC)                                            # get time stamp of frame
    print "time stamp:",tt                                                           # display timestamp
    ...
    if len(contours) > 0:                                                             # only if contours are present
        for cnt in contours:                                                          # iterate to find the area of the largest contour
            area=cv2.contourArea(cnt)
            #if (area>largest_area):
            largest_area=largest_area+area

    print "area:",largest_area                                                        # print the largest area
          
    if largest_area>525.6:                                                             # to detect if led is ON or OFF                                                        
        if ledstate==0:
            continue
